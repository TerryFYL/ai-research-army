...
**设计理念**: 当研究问题有多个合法统计方法时，不依赖主观判断"选一个"，
而是并行执行所有候选方法，通过量化评分自动选出最优方法进入正式三件套输出。
...
| 研究场景 | 候选方法 A | 候选方法 B | 候选方法 C |
|---------|-----------|-----------|-----------|
| 时间-事件结局，多因素 | Cox 比例风险 | Fine-Gray 竞争风险 | — |
| 时间-事件结局，小样本（N<200） | Cox（简化协变量）| KM 分层+log-rank | — |
| 二分类结局，多因素 | Logistic 回归 | 倾向性评分匹配+回归 | — |
| 连续结局，组间比较 | 多元线性回归 | ANCOVA | — |
| 纵向重复测量数据 | 混合效应模型（LME）| GEE | — |
| 诊断准确性 | ROC/AUC+单截断值 | IDI/NRI 改进指数 | — |
| 生存分析+竞争事件>15% | Cox 比例风险 | Fine-Gray 竞争风险 | — |
| 复杂调节/中介 | 普通回归+交互项 | Bootstrap 中介分析 | — |
...
**分析结果必须版本化输出**，防止重跑时覆盖旧结果导致稿件-数据不同步。

### 输出目录结构

```
outputs/
  analysis_20260311_1430/     ← 第一次跑
    full_analysis_report.md
    tables/
    figures/
  analysis_20260312_0900/     ← 重跑（排除异常值后）
    full_analysis_report.md
    tables/
    figures/
  results_current → analysis_20260312_0900/  ← 软链接指向最新
  analysis_changelog.md       ← 变更日志
```

### 版本化规则

1. **每次分析**输出到 `outputs/analysis_{YYYYMMDD_HHMM}/`
2. 同时更新 `results_current` 软链接指向最新目录
3. 重跑时**不删除**旧版本目录，保留完整历史
4. 在 `analysis_changelog.md` 记录每次变更的原因和关键变化
...
| 时间 | 版本目录 | 变更原因 | 关键变化 |
|------|---------|---------|---------|
| 2026-03-11 14:30 | analysis_20260311_1430 | 初始分析 | — |
| 2026-03-12 09:00 | analysis_20260312_0900 | 排除 3 例异常值 | HR 1.45→1.52, p .003→.001 |
```

### 与 NTC 的联动

NTC v2 的 LOCATE 步骤搜索 `results_current/` 目录下的 CSV 文件。
当分析重跑后，`results_current` 自动指向最新结果，NTC 即可发现稿件中过时的数字。
...
```bash
# R Docker 位置
~/.claude/skills/statistical-analysis/docker/
...
- **v4.2** (2026-03-11): 结果版本管理
  - 新增"结果版本管理"章节: 每次分析输出到带时间戳的目录，不覆盖旧版本
  - `results_current` 软链接指向最新结果，NTC v2 自动搜索最新版本
  - `analysis_changelog.md` 记录每次重跑的原因和关键变化

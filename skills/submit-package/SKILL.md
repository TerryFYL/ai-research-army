...
## 概述
科研军团流水线的倒数第二站。将论文及所有配套材料组装为期刊可直接上传的投稿包。执行前置门控检查，确保质量审查已通过且引用已验证，然后将 Markdown 转换为 Word 格式并组装全部投稿文件。最终对外交付由 `/delivery` 收口。
...
5. **投稿包不是最终终态**：`submission_package/` 是 `/delivery` 的输入，而不是最终对外交付目录
6. **最终检查**：组装完成后逐一打开每个文件确认无误，再进入 `/delivery`
